-- WSOPTV OTT Database Schema - Initial Tables
-- Version: 2.0.0
-- Created: 2026-01-19
-- Reference: ADR-0002-database-schema-design.md (WSOP 브랜드 기반 v2.0)

-- ============================================================================
-- WSOP BRAND HIERARCHY
-- ============================================================================
-- WSOP (브랜드)
-- ├── WSOP (본대회 - Las Vegas)
-- ├── WSOP Paradise
...
-- series (시리즈) - WSOP 브랜드 하위의 시리즈
-- v2.0: circuits → series 변경
...
-- sessions (세션/데이) - v2.0: tournaments → sessions 변경
...
COMMENT ON TABLE sessions IS '이벤트의 각 Day 또는 Flight 세션';
...
CREATE TABLE IF NOT EXISTS content (
    ...
    -- Type
    content_type VARCHAR(50) NOT NULL,             -- 'live', 'vod', 'quick_vod', 'highlight', 'archive', 'clip'
...
CREATE TABLE IF NOT EXISTS content_streams (
    ...
    quality VARCHAR(20) NOT NULL,                  -- '1080p', '720p', '480p', '360p', '240p', 'auto'
...
CREATE TRIGGER update_content_streams_updated_at
    BEFORE UPDATE ON content_streams
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

COMMENT ON TABLE content_streams IS 'HLS/DASH 스트림 URL (DRM 포함)';

-- key_hands (주요 핸드 - PRD-0006)
CREATE TABLE IF NOT EXISTS key_hands (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TRIGGER update_key_hands_updated_at
    BEFORE UPDATE ON key_hands
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TABLE IF NOT EXISTS users (
    ...
    subscription_status VARCHAR(20) DEFAULT 'active', -- 'active', 'cancelled', 'expired', 'pending'
...
CREATE TABLE IF NOT EXISTS subscriptions (
    ...
    -- Status
    status VARCHAR(20) DEFAULT 'active',           -- 'active', 'cancelled', 'expired', 'refunded', 'pending'
...
CREATE TABLE IF NOT EXISTS watch_history (
    ...
    -- Session Info
    device_type VARCHAR(50),                       -- 'web', 'ios', 'android', 'samsung_tv', 'lg_tv', 'roku', 'fire_tv', 'other'
...
-- v2.0: circuit_points → series_points 변경
-- ============================================================================

-- series_points (시리즈 포인트)
