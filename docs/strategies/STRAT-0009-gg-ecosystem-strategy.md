...
### 6.1 MVP vs Phase 2+ 기능 분류 (2026-02-02 업데이트)

> **전략 변경**: Vimeo 기본 템플릿 기반 MVP 론칭 → 고급 기능은 Phase 2+ 검토

| 기능 | MVP | Phase 2+ | 생태계 가치 |
|------|:---:|:--------:|------------|
| **Single View** | ✅ | - | 기본 시청 경험 제공 |
| **Table Multi-view** | ❌ | ⚠️ 검토 | 포커 관전 놀이터 (구현 난이도 높음) |
| **Player Cam** | ❌ | ⚠️ 검토 | 아이돌 팬덤 (추가 인프라 필요) |
...
> **핵심 인사이트**: Multi-view/Player Cam의 생태계 가치는 인정하나, **구현 난이도 대비 ROI가 불확실**하여 MVP 필수 요건에서 제외. 사용자 데이터 수집 후 Phase 2에서 재검토.

---

## 7. 프리미엄 구독 혜택 전략

> **전략 변경 (2026-02-02)**: Multi-view는 MVP에서 제외. **대안 프리미엄 혜택**으로 Plus+ 가치 제안.
...
| | 광고 제거 | Plus+ 전용 | ✅ |
| **Phase 2+** | Multi-view | 여러 테이블 동시 시청 | ❌ |
| | Player Cam | 플레이어 직캠 | ❌ |
| | StatsView | 통계 오버레이 | ❌ |
...
| 버전 | 날짜 | 작성자 | 내용 |
|------|------|--------|------|
| 1.1 | 2026-02-02 | Claude Code | Vimeo 기반 MVP 전략 반영, Multi-view Phase 2+ 이동 |
